...
use crate::app_state::{AppState, ImportMode};
...
pub(crate) fn import_menu_popup(ui: &mut Ui, state: &mut AppState, theme: &PlyTheme) {
    ...
    ui.element()
        ...
        .children(|ui| {
            import_option(ui, state, theme, lang, ImportMode::Workspace);
            ...
            import_option(ui, state, theme, lang, ImportMode::Tmx);
        });
}
    ...
    ui.element()
        ...
        .children(|ui| {
            if ui.just_released() {
                ...
                platform::launch_directory_picker(mode_str);
            }
            let icon = state.icon_cache.get(icon_id);
            ui.element()
                .width(fixed!(18.0))
                .height(fixed!(18.0))
